...
type ExportResponse = {
  ...
  result?: { at_bookmark?: string; result?: { signed_url?: string; filename?: string } };
};
...
export class BackupWorkflow extends WorkflowEntrypoint<Env, BackupPayload> {
  async run(event: WorkflowEvent<BackupPayload>, step: WorkflowStep): Promise<void> {
    ...
    await step.do("download and store D1 export", { retries: { limit: 12, delay: "30 seconds", backoff: "constant" } }, async () => {
      const response = await fetch(endpoint, { method: "POST", headers, body: JSON.stringify({ output_format: "polling", current_bookmark: bookmark }) });
      const body = await response.json<ExportResponse>();
      const artifact = body.result?.result;
      if (!response.ok || !artifact?.signed_url || !artifact.filename) throw new Error(body.errors?.[0]?.message ?? "D1 export is not ready");
      const dump = await fetch(artifact.signed_url); if (!dump.ok || !dump.body) throw new Error("Could not download D1 export");
      const scheduledAt = new Date(event.payload.scheduledTime ?? event.timestamp).toISOString();
      const date = scheduledAt.slice(0, 10), dailyKey = `daily/${date}/${artifact.filename}`, metadata = { bookmark, exportedAt: scheduledAt };
      const keys = [dailyKey];
      if (date.endsWith("-01")) keys.push(`monthly/${date.slice(0, 7)}/${artifact.filename}`);
