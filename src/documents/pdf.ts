...
import { defaultContractorSettings, formatAddress } from "../domain/addresses";
import type { AdjustmentInput, ContractorSettingsInput, EstimateSnapshot } from "../domain/contracts";
...
export function estimateHtml(s: EstimateSnapshot, contractor: ContractorSettingsInput = defaultContractorSettings): string {
...
Customer ID: ${esc(s.customer.displayId)}</p></div></section><section class="project"><h2>PROJECT DETAILS</h2><table><tbody><tr><th>Job ID</th><td>${esc(s.job.displayId)}</td><th>Job name</th><td>${esc(s.job.name)}</td></tr><tr><th>Address</th><td colspan="3">${esc(s.job.address)}</td></tr><tr><th>Scope</th><td colspan="3">${esc(s.job.scope)}</td></tr></tbody></table></section>` : "";
    const contractorAddress = formatAddress(contractor)?.replace(/\n/g, " · ") ?? "";
    return `<section class="page">${header}<main>${intro}${chargeTable(lines, !first)}${last ? `${totals}${notes}<section class="contact"><h2>CONTACT</h2><p><strong>${esc(contractor.companyName)}</strong> · ${esc(contractor.contractorName)}</p><p>${esc(contractorAddress)} · ${esc(contractor.phone)} · ${esc(contractor.email)}</p></section>` : ""}</main>${footer}</section>`;
  }).join("");
